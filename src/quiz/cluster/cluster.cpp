...
{
	processed[pt_index] = true;
	cluster.push_back(pt_index);
	std::vector<int> nearbyPoints = tree->search(points[pt_index], distanceTol);
	for (int pt_id : nearbyPoints)
	{
		if (processed[pt_id] == false)
		{
			proximity(points, cluster, tree, processed, pt_id, distanceTol);
		}
	}
}
											   ...
											   float distanceTol)
{
	...
	for (int pt_index = 0; pt_index < points.size(); pt_index++)
	{
		if (processed[pt_index] == false)
		{
			std::vector<int> cluster;
			proximity(points, cluster, tree, processed, pt_index, distanceTol);
